...
def test_path_efficiency(empty_map,goal_State):
    ...
    for i in range(len(path)-1):
        total_dist += math.sqrt((path[i+1][0]-path[i][0])**2 + (path[i+1][1]-path[i][1])**2)
...
def test_planner_throughput_and_binning():
    ...
    compression_ratio = call_counts["snaps"] / nodes_expanded if nodes_expanded > 0 else 0
    ...
    assert nodes_per_second > 500, "Planner is running too slow! Check for geometric bottlenecks."
    
    # 2. Ensure binning is actually working
    # If compression is 1.0x, it means every single move became a new state (no pruning)
    assert compression_ratio > 1.0, "Binning is not pruning any states. Check snap_to_grid resolution."
...
def test_goal_tolerance_acceptance(goal_State):
    ...
    final_node = path[-1]
    dist_to_goal = math.sqrt((final_node[0]-goal[0])**2 + (final_node[1]-goal[1])**2)

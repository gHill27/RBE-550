...
class Truck(Vehicle):
        ...
        for direction in [1, -1]:
            for phi in motion_primatives.steer_options:
                res = motion_primatives.get_primitive(
                    current_state=current_state, phi=phi, direction=direction
                )
                if res:
    ...
    def get_footprint(self, x, y, t0, t1) -> Polygon:
        ...
        truck_poly = translate(
            rotate(truck_base, t0, origin=(0, 0), use_radians=False), x, y
        )
        ...
        # Rotate around the HITCH (0,0), then move to world (x,y)
        trailer_poly = translate(
            rotate(trailer_base, t1, origin=(0, 0), use_radians=False), x, y
        )

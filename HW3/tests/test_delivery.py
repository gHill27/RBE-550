...
@pytest.mark.parametrize("state, expected_safe", [
...
@pytest.mark.parametrize("state, description", [
    ((0.2, 5.0, 0.0), "Left side poke-out"),     # Center is at 0.2, but width/2 is 0.35 -> -0.15 (Out!)
    ((11.8, 5.0, 0.0), "Right side poke-out"),   # 11.8 + 0.35 = 12.15 (Out!)
    ((5.0, 0.2, 90.0), "Top side poke-out"),     # Rotated 90 deg, length becomes height
...
def test_strict_boundaries(mock_delivery, state, description):
    ...
    is_safe = mock_delivery.is_state_valid(state)
    assert is_safe is False, f"Cheating detected! {description} at {state} should be invalid."

    
# def test_impossible_maze_with_visual_debug(mock_delivery):
#     """
#     Creates an impossible wall and verifies the planner gives up.
#     If it fails, we can trigger the visualizer to see the 'leak'.
#     """
#     # 1. Create a wall from x=0 to x=12 at y=6
#     # We include fractional steps to ensure no 'diagonal' squeezing is possible
#     wall = [(6, x) for x in range(13)] 
    
#     with patch('Vehicles.Map') as MockMap:
#         fake_map = MockMap.return_value
#         fake_map.goal_pos = (6.0, 10.0)
#         fake_map.obstacle_coordinate_list = wall
#         fake_map.grid_size = 12

#         robot = Delivery()
#         robot.start_pos = (6.0, 2.0, 0.0)
#         robot.prepare_obstacles(wall)

#         # Run the plan
#         path = robot.plan(goal=fake_map.goal_pos)

#         # If the path is NOT None, the robot cheated! 
#         # We can call the visualizer here manually to debug the 'leak'
#         if path is not None:
#             print("\nDEBUG: Robot found a ghost path!")
#             robot.viz.show_final(path, {}, wall, fake_map.goal_pos)
            
#         assert path is None, "Planner found a path through a solid wall. Boundary check is leaking!"
import time
...
@pytest.mark.parametrize("state, expected, reason", [
    # --- BOUNDARY CHECKS ---
...
def test_impossible_maze_fails_correctly(mock_delivery):
    ...
    with patch('Vehicles.Map') as MockMap:
        ...
        assert path is None, "Planner 'cheated' or found a ghost path through a solid wall!"